...
# Example: ruby cut-music.rb MP3_DIR CUT_DIR START_TIME END_TIME
MP3_DIR = ARGV[0]
CUT_DIR = ARGV[1]
START_TIME = ARGV[2]
END_TIME = ARGV[3]
...
if MP3_DIR.nil? || CUT_DIR.nil? || START_TIME.nil? || END_TIME.nil?
  puts "Usage: ruby cut-music.rb MP3_DIR CUT_DIR START_TIME END_TIME"
  exit
end
...
# Check if the CUT_DIR exists
if Dir.exist?(CUT_DIR) == false
  Dir.mkdir(CUT_DIR)
end
...
mp3_files.each do |mp3_file|
  cut_file = "#{CUT_DIR}/#{File.basename(mp3_file, ".mp3")}.mp3"
  system("ffmpeg -i '#{mp3_file}' -ss #{START_TIME} -t #{END_TIME} -acodec copy '#{cut_file}'")
end
